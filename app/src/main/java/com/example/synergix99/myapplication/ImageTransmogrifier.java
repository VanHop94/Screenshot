    ...
    public void onImageAvailable(ImageReader reader) {
        ...
        if (image!=null) {
          ...
          //  byte[] newPng=baos.toByteArray();
            ...
            File file = new File(path, "FitnessGirl"+".png"); // the File to save to
            try {
                fOut = new FileOutputStream(file);
                Bitmap cropped=Bitmap.createBitmap(latestBitmap, 0, 0,
                        width, height);

                cropped.compress(Bitmap.CompressFormat.PNG, 100, fOut);
                fOut.flush();
                fOut.close();

                MediaStore.Images.Media.insertImage(svc.getContentResolver(),file.getAbsolutePath(),file.getName(),file.getName());

                Toast.makeText(svc.getApplicationContext(), "Screenshot", Toast.LENGTH_LONG).show();
            } catch (Exception e) {
                e.printStackTrace();
            }
